...
Recorded at Task 11 CP2 close-out.

| Metric | Value |
|---|---|
| Plan lines | 1,161 |
| Checkpoints (plan body) | 31, matching the header's budget exactly |
| Lines/checkpoint | 1,161 / 31 ≈ 37 — under the header's ≤60 target, and close to 4a's ≈36 |
| Commits landed | 32 (31 `feat`/`fix`/`test` + this `docs` close-out) against the plan's flat 31 — net +1, from two checkpoints merging into one commit (T9 CP1+CP2, inseparable at the implementation level: a router that dispatches without also replying to errors isn't a meaningful intermediate state) offset by two extra commits a checkpoint's own verification work produced (a `round.Broadcaster` stub fix while closing out T6, and a coverage-closing test at T11 CP2 — see below) |
| Checkpoints un-batched into separate verify/commit steps | 0 — every checkpoint's combined "implement, then verify-and-commit" step ran as written |
| Checkpoints that passed immediately | 6: T3 CP2 (named in advance), T3 CP3 and T6 CP3 (both named in advance, both verified genuine via disable-and-confirm), T7 CP2 (not named, passed via CP1's already-correct terminal-check guard; the plan's belt-and-braces `ErrAlreadySettled` tolerance was still added per its own contract), T8 CP2 (not named, guest/never-joined paths fell out of CP1's `if guest { return }` and store's own `ErrNotFound`), T10 CP1 (named, e2e composition of Tasks 1–9) |
| Real defects/plan gaps found during execution | 6 — see below |

**Defects and plan gaps, in the order hit:**

1. **`LockIn` validation vs. fast tests.** Task 2 CP3 set `MinLockIn = 3s`, but Task 3's own checkpoint text specifies test lock windows of 100ms–2s, and Task 10 CP1's e2e spec literally says `lock_in_ms: 500`. Both directly fail `Open`'s real validation. Resolved two ways: Task 3/7 timer tests bypass `Open` (`store.CreateRound` + a direct `svc.watch(...)` call) rather than weakening the validation; the e2e test uses `3000` (`MinLockIn` itself) with a wider read timeout instead of 500.
2. **`internal/round` cannot import `internal/ws`.** Task 2 CP1's contract text says "reuse `internal/ws`'s `Encode`", but Task 9's own Interfaces block establishes `ws` imports `round` (for the message router) — `round` importing `ws` too would cycle. `round.EncodeEnvelope` mirrors `ws.Envelope`'s wire format locally instead (`wager` reuses `round`'s copy, so there's still one encoder on that side of the boundary, not two). Task 9's constraint is a hard compilation rule; it wins over Task 2's looser wording.
3. **`internal/ws/e2e_test.go` must be `package ws_test`, not `package ws`.** The end-to-end test needs `internal/httpapi` for the REST half of the flow, and `httpapi` imports `ws` — an *internal* test file (`package ws`) pulling in `httpapi` is a real build cycle (confirmed via `go vet`), unlike the "internal test importing a package that imports the plain package under test" pattern that works fine elsewhere in Go. The external test package avoids it, at the cost of a few duplicated test-only helpers (`e2eTestStore`, `e2eTestID`) that couldn't reuse `package ws`'s own unexported ones.
4. **Typed-nil-in-interface panic (T11 CP1).** `registerWSRoutes` assigned a possibly-nil `*round.Service` directly into the `ws.SessionEnder` interface parameter. A nil `*round.Service` wrapped in an interface is not the nil interface — `Handler`'s own `sessions != nil` guard passed, and `EndSession` panicked on a nil receiver. This broke several of `httpapi`'s own pre-existing WS tests that build `Deps` without a round service. Fixed by only assigning the interface value when `d.Rounds` is actually set. A real bug this wiring checkpoint was worth doing specifically to surface.
5. **Rate-limit test cross-contamination (T4 CP4).** Several `internal/wager` test functions reused literal user IDs (`"u1"`, `"u2"`) across the file. The wager throttle's Redis key is scoped by user ID alone, not by room, so hits from earlier tests in the same 10-second window bled into `TestPlaceRateLimited`, which failed only under the full package run, never in isolation. Fixed by switching every test to per-test-generated IDs.
6. **Coverage gap closed at close-out.** `internal/round` measured 78.1% under the `-coverpkg=./...` profile (CLAUDE.md's own note: the per-package figure can be a measurement artifact, so this profile is the one that counts) — just under the phase's 80% floor. The gap was `watch`'s second cancellation branch (the refund wait's `ctx.Done()` case): Task 7 CP3's `TestTimerCancelled` only ever reached the *first* select (the lock wait), since it cancels before any lock happens. Added `TestTimerCancelledAfterLock`, which cancels between lock and refund; round now measures 80.5%. Verified via the same disable-and-confirm procedure as the lock-wait branch, with the same finding: a cancelled context also makes the subsequent Redis call fail harmlessly, so the assertion can't distinguish the explicit `select` from its absence — the `select` is kept anyway, since it stops the goroutine immediately instead of sleeping out the full window first.

**Verdict for `writing-plans-tuned` vs. `writing-plans`, two data points in:** 4a landed 0 un-batched checkpoints and 5 premature passes (all named in advance, all confirmed genuine); 4b landed 0 un-batched checkpoints and 6 premature passes (4 of 6 named in advance). The un-batching number the experiment was designed to produce has now held at zero across a transport phase and a money-movement phase with real cross-package wiring — different enough problem shapes that this isn't one lucky case. The premature-pass rate (2 unnamed out of 6 in 4b, vs. 0 unnamed out of 5 in 4a) suggests the self-review's "checkpoint realness" pass still slightly under-predicts which checkpoints will turn out to be interaction tests rather than genuine new-code tests — worth a closer look in `writing-plans` before the next phase, but not a reason to abandon the tuned format.
