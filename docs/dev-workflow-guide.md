...
| Situation | Use |
|---|---|
| Writing CallIt's `CLAUDE.md` | Do this **after the plan**, once repo layout/schemas are decided (see prior discussion in this session) — not before. Can bootstrap via `init` skill or `codebase-onboarding` once there's a first slice of code to reverse-engineer conventions from |
| Installing stack-specific rule packs (Go idioms, Postgres, etc.) once the stack is locked in by the plan | Copy from `/home/chikara/projects/ECC/rules/golang`, `.../postgres` etc. into `call_it/.claude/rules/ecc/` (project-local, per the ecc-survey's rules section — avoids leaking Go conventions into unrelated sessions) |
| Pulling in stack-specific skills once the plan locks the stack | Bucket 3 picks per `ecc-survey.md`: `redis-patterns`, `postgres-patterns`, `golang-*` (patterns/testing/tdd/verification), `react-patterns`/`nextjs-turbopack`, `database-migrations` — install project-locally into `call_it/.claude/skills/`, not globally |
