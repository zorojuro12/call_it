...
func TestMain(m *testing.M) {
	...
	if err := client.Ping(context.Background()).Err(); err != nil {
		log.Fatalf("round: cannot reach Redis at %s (db %d): %v — run `make up` and retry", testRedisAddr, testDB, err)
	}

	if err := client.FlushDB(context.Background()).Err(); err != nil {
		log.Fatalf("round: FLUSHDB on db %d failed: %v", testDB, err)
	}

	if err := client.Close(); err != nil {
		log.Fatalf("round: closing setup client failed: %v", err)
	}
